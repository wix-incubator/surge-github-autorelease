...
program
  ...
  .option(
    '-d, --domain [domain]',
    'Domain to deploy to, example: wix-wix-style-react',
  )
  .option('-o, --repo-owner [type]', 'Repo Owner Name example: wix (optional)')
  .option(
    '-n, --repo-name [type]',
    'Repo Name example: wix-style-react (optional)',
  )
...
if (sourceDirectory && rootPath && domain) {
  console.log(
    `Called with repoOwner: ${repoOwner} repoName: ${repoName} sourceDirectory: ${sourceDirectory} PR number: ${pr} Root path: ${rootPath} Domain: ${domain}`,
  );
  sgAutorelease({
    repoOwner,
    repoName,
  ...
  console.log(
    'Usage: surge-github-autorelease -o wix -n wix-style-react -s storybook-dist -b . -p 1455 -t ya65s2sjhd -d wix-wix-style-react',
  );
